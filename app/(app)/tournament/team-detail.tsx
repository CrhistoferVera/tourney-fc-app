...
import { getTeamById, getTeamInTournament, MyTeam } from '../../../services/teamsService';
...
function Header({ nombre, onBack }: { readonly nombre: string; readonly onBack: () => void }) {
  return (
...
export default function TeamDetailScreen() {
  const { teamId, torneoId } = useLocalSearchParams<{ teamId: string; torneoId?: string }>();
  ...
  const fetchEquipo = useCallback(async () => {
    ...
    try {
      const data = torneoId
        ? await getTeamInTournament(teamId, torneoId)
        : await getTeamById(teamId);
      setEquipo(data);
    } catch {
      if (torneoId) {
        try {
          const fallback = await getTeamById(teamId);
          setEquipo(fallback);
        } catch (error_: any) {
          showError('Error', error_.message ?? 'No se pudo cargar el equipo');
          setEquipo(null);
        }
      } else {
        showError('Error', 'No se pudo cargar el equipo');
        setEquipo(null);
      }
    }
  }, [teamId, torneoId]);
  ...
  if (loading) {
    return (
      <View className="flex-1 bg-mist">
        <Header nombre="Detalle del equipo" onBack={() => router.back()} />
  ...
  return (
    <View className="flex-1 bg-mist">
      <CustomAlert {...alertState} onConfirm={alertState.onConfirm} onCancel={hideAlert} />
      <Header nombre={equipo.nombre} onBack={() => router.back()} />
