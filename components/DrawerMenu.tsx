...
import {
  ...
  Dimensions, TouchableWithoutFeedback, Image, Easing,
} from 'react-native';
    ...
    <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 999 }}>
      <TouchableWithoutFeedback onPress={onClose}>
        <Animated.View style={{
          ...
          backgroundColor: 'rgba(15,26,20,0.55)', opacity,
        }} />
      ...
      <Animated.View style={{
        ...
        {/* Perfil */}
        <TouchableOpacity
          onPress={() => { onClose(); setTimeout(() => router.push('/(profile)'), 250); }}
          activeOpacity={0.85}
        >
          <View className="bg-primary pt-14 pb-5 px-5 flex-row items-center">
            {usuario?.fotoPerfil ? (
              <Image
                ...
                style={{ width: 52, height: 52, borderRadius: 26, borderWidth: 2, borderColor: 'rgba(255,255,255,0.3)' }}
              />
            ) : (
              <View style={{
        ...
        <View className="px-3 pt-5">
          <Text style={{
            color: '#3D4F44', fontSize: 11, fontFamily: 'Inter_500Medium',
            letterSpacing: 1, textTransform: 'uppercase', paddingHorizontal: 8, marginBottom: 6,
          }}>
            Menú
          </Text>
          {SECTIONS.map((s, i) => {
